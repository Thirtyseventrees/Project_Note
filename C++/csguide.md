...
- [C++编译过程](#c编译过程)
  ...
  - [静态链接](#静态链接)
    - [空间与地址分配](#空间与地址分配)
    - [静态库（通常为以.a为后缀的文件）](#静态库通常为以a为后缀的文件)
  - [动态链接共享库（通常为以.so为后缀的文件）](#动态链接共享库通常为以so为后缀的文件)
...
- [内存管理](#内存管理)
  - [动态链接与共享库](#动态链接与共享库)
  - [malloc内存分配，free释放内存时为什么知道内存大小](#malloc内存分配free释放内存时为什么知道内存大小)
  - [AddressSanitizer的实现原理](#addresssanitizer的实现原理)
- [用户态和内核态](#用户态和内核态)
...
- [计网](#计网)
  ...
  - [CLOSE\_WAIT](#close_wait)
    - [出现大量CLOSE\_WAIT怎么排查](#出现大量close_wait怎么排查)
...
- [智能指针](#智能指针)
  ...
  - [怎么在main函数开始前打印一段日志](#怎么在main函数开始前打印一段日志)
- [C++ OOP](#c-oop)
  - [虚函数，虚表，虚指针](#虚函数虚表虚指针)
...
我们也可以把所有相关的目标模块打包成一个单独的文件，称为静态库，它可以用作链接器的输入，当链接器构造一个输出的可执行文件时，它只复制静态库里被应用程序引用的模块

静态库的优点在于我们可以把一堆实现放在一个文件里直接链接，而不是把每个单独的实现一个一个链接，而静态库还能做到对于一堆实现的静态库，在链接时链接器只会复制被程序引用的模块，从而减少可执行文件在磁盘和内存中的大小

对于静态库，如果静态库的代码改变了，需要重新编译静态库并且显式地重新链接程序

## 动态链接共享库（通常为以.so为后缀的文件）

虽然静态库优化了很多问题，但是仍然有缺点，首先就是静态库的更新需要显式地重新链接程序，其次如果一个静态库的某些模块被多个程序引用了（例如大部分程序都会使用的`printf`和`scanf`），那么在运行时，这些函数的代码会被复制到每个运行程序的代码段中，从而造成对内存的浪费

因此采用动态链接的共享库就用来解决这些问题

共享库是一个目标模块，在运行或者加载时，可以加载到任意的内存地址，并和一个在内存中的程序链接起来。这个过程被称为动态链接

共享库是通过以下两种方式来完成共享的：  
1. 对于一个库只有一个.so文件，所有引用该库的可执行文件共享这个.so文件中的代码和数据，而不是像静态库那样复制嵌入到各自的可执行文件中去
...
CPU访问的虚拟地址没有有效映射，触发异常，内核接入建立映射

当一条指令访问的内存在页表中没有有效映射时：
1. 切换为内核态
2. 保存用户寄存器和进程状态
3. 判断缺页类型（如果是合法访问则继续处理，非法访问则进程中断）
4. 把存储在磁盘的页加载到物理内存中或者对未分配的页分配内存
5. 更新页表和TLB
6. 返回用户态
...
1. 内存  
    多线程是共享内存的，多进程每个进程都有自己独立的地址空间  
    因此对于多线程，共享数据方便，但是容易数据竞争  
    对于多进程，数据互不共享

2. 通信  
   多线程因为共享内存，所以直接读写就能通信（但是要加锁）  
   而多进程必须进程间通信（IPC），例如管道，共享内存，消息队列，socket  
   **注：** 多进程因为地址空间不同，即使通过共享内存的方式通信，也不能传递裸指针
...
4. 稳定性  
   多线程的情况下，一个线程崩溃会导致整个进程崩溃  
   而多进程的情况下，一个进程崩溃并不会影响另一个进程

所以在浏览器中，每一个页面都会开一个进程，不会因为某一个网页的崩溃导致整个浏览器的崩溃

对于高频共享数据，高并发IO建议使用多线程  
而对于高可靠，CPU密集计算，需要安全隔离的情况建议用多进程

# 内存管理

我们需要确保每个进程都有一个单独的内存空间，需要确定一个进程可以访问的合法地址范围  
通过两个寄存器，通常为基地址寄存器和界限地址寄存器，前者存储最小的合法的物理内存地址，后者指定了范围的大小

CPU生成的地址通常称为逻辑地址（虚拟地址），而内存单元看到的地址（即加载到内存地址寄存器的地址）通常称为物理地址  
从虚拟地址空间到物理地址的运行时映射是由内存管理单元MMU的硬件设备来完成的

采用动态加载时，一个程序只有在调用时才会被加载。所有程序都以可重定位加载格式保存在磁盘上。  
当一个程序需要调用另一个程序时，调用程序首先检查另一个程序是否已经被加载，如果没有，可重定位链接程序会加载所需的程序到内存

## 动态链接与共享库

## malloc内存分配，free释放内存时为什么知道内存大小

`malloc`在返回的指针附近保存了额外的元数据，里面记录了这块内存的大小等信息  
当我们调用malloc申请一块内存时，内存分配器内部可能实际分配的是：
```
+----------------------+-------------------------+
| allocator metadata   | usable memory           |
| size, flags, etc.    | returned to user        |
+----------------------+-------------------------+
                       ^
                       p
```
也就是说p指向的是用户可用区域，而在p前面通常有一段隐藏的头部信息  
当调用free的时候，分配器会根据p向前偏移一段距离，找到这个chunk的头部，然后从header中读出内存块的大小

因此free不能释放不是malloc返回的指针，因为调用free的时候它会假设传进来的指针是malloc分配的，从而向前去寻找metadata，如果传入的是非法地址，则会读取到错误的header
...
int main() {
    ...
    auto* q = (std::uintptr_t*)p;

    printf("word before p[-1] = 0x%lx\n", (unsigned long)q[-1]);
    printf("word before p[-2] = 0x%lx\n", (unsigned long)q[-2]);
...
p = 0x55d2bcb4a2a0
usable = 104
word before p[-1] = 0x71
word before p[-2] = 0x0
```

使用malloc，free创建数组内存时，处理方式和之前一样，编译器并不在乎你怎么在逻辑上把这块内存分成了多个对象组成的数组，它只在乎这是一块完整的内存

而new[](例如：`int *a = new my_class[100]`)就有点不一样  
对于int这样的平凡类型，底层可能和malloc类似，但对于有析构函数的类数组，delete[]必须知道数组中有多少个元素，因为对于每个元素它都要调用析构函数，因此，new[]通常还会额外保存一个array cookie，记录数组元素个数  
```
低地址                                                         高地址
+------------------------+-------------------+------------------+
| allocator metadata     | array cookie      | A[0] A[1] ...    |
| malloc/new metadata    | element count     | user array       |
+------------------------+-------------------+------------------+
                                            ^
                                            arr
```
...
```
p = 0xdb37eb8
word before p[-1] = 0x64
word before p[-2] = 0x651
```
```
q[-1] 可能是 array cookie，也就是元素个数 100
q[-2] 可能是 malloc chunk 的 size 字段
```



## AddressSanitizer的实现原理

https://github.com/google/sanitizers/wiki/AddressSanitizerAlgorithm

总的来说Asan的实现是通过编译器插桩，重载malloc，free，shadow memory的手段来实现的

1. 编译器插桩
   当我们在编译时加上对应的指令时，对于代码中每一段对内存的读写，都会在前面插入额外检查，例如：  
   当源码中有：  
   ```cpp
   int x = arr[i];
   ```
   编译后的逻辑大致会变成：
   ```cpp
   if (asan_check_read(&arr[i], sizeof(int)) == false) {
        report_error();
   }
   int x = arr[i];
   ```
...
2. Shadow memory  
    那我们要怎么检测访问的内存是否正确呢  
    我们需要记录下每一块内存的状态，每次访问内存时通过看这块内存的状态来判断访问是否合法  
    如果我们对每个字节都单独存一个状态，这样对内存的占用开销会非常大  
    所以Asan使用一个shadow字节描述八个真实内存的可访问状态  
    ```
    1 byte shadow memory : 8 bytes application memory
    ```  
    映射关系大概是：
    ```
    ...
    shadow byte用来表示对应的8字节真实内存是否可访问  
    ```
    shadow = 0      表示对应的 8 字节全部可访问
    shadow = 1~7    表示前 N 个字节可访问，后面的不可访问
    shadow < 0      表示这 8 字节整体不可访问，原因可能是 redzone/free 等
    ```
    例如：  
    ```cpp
    char* p = (char*)malloc(10);
    ```
    Asan可能分配更大的区域，并在周围放redzone：
    ```
    真实内存布局：

    [ left redzone ][ user 10 bytes ][ right redzone ]
                    ^
                    p
    ```
    对应的shadow memory：
    ```
    真实内存:   R R R R R R R R | A A A A A A A A | A A R R R R R R | R R R R R R R R
    shadow:     poisoned       | 0               | 2               | poisoned
    ```
...
3. 重载malloc和free函数
    ...
    1. 实际申请更大的块
    2. 在用户区域左右加 redzone
    3. 设置 shadow memory：用户区标记为可访问，redzone 标记为不可访问
    4. 返回用户区指针

    free:
    1. 检查指针是否合法
    2. 将用户区 shadow memory 标记为不可访问
    3. 把这块内存放入 quarantine，暂时不立刻复用
    ```


# 用户态和内核态

当计算机执行用户应用时，处于用户态，受限制的运行环境  
而内核态是拥有全部权限的运行环境

## 用户态到内核态的切换

只要需要受保护的内核资源或特权操作，CPU就必须从用户态切换到内核态  
主要三种来源：  
1. 系统调用  
   用户程序主动请求内核服务  
   例如：read, write, fork, mmap  
2. 异常  
   程序执行出错，CPU被动陷入内核  
   例如：缺页异常，除零错误，非法指令  
3. 中断  
   外部设备通知CPU  
   例如：磁盘IO完成，网络数据到达，时钟中断（定时器触发->用于调度 ）
...
IO复用指的是一个进程/线程，可以同时监听多个文件描述符，等其中任何一个准备好进行IO，再去处理它  
复用的是：同一个等待机制，去管理多个IO对象的就绪检测能力
...
假设我需要同时处理很多socket连接，如果不用IO复用，最直观的办法是：  
- 一个连接配一个线程
- 每个线程阻塞在`read()`上等待数据

而IO复用将多个IO通道复用在一个统一的等待入口上
...
TIME_WAIT是TCP主动关闭连接的一方，在最后一次发送ACK报文之后进入的状态
...
2MSL的原因是客户端最后一次发送ACK到服务端如果丢失，服务端会重发FIN报文，这里ACK/FIN这一组来回报文最大时间就是2MSL
...
CLOSE_WAIT存在是为了被动关闭的那一方有时间把剩余数据处理完  

全双工通信两个方向是独立关闭的  
客户端发送FIN给服务端时表示客户端这边不再发送数据了，但还能接收  
服务端接收到客户端的FIN进入CLOSE_WAIT状态，让服务端有时间处理剩余数据并调用close  

### 出现大量CLOSE_WAIT怎么排查

出现大量CLOSE_WAIT的本质原因是对端已经发了FIN, 但服务端没有调用`close()`（或没有把fd释放掉）

首先可以用`netstat`找到对应出现CLOSE_WAIT的进程  
```
netstat | grep CLOSE_WAIT
```

然后排查对应程序是否在各种情况下有没有调用`close()`（read()返回0，错误处理，线程结束前释放fd等）
...
HTTP 是基于 TCP/IP 协议族中的传输层协议来实现的应用层协议。

因为HTTP需要数据完整，顺序正确，出错了要重传  
而TCP的三次握手，重传机制，滑动窗口，拥塞控制提供了这些能力

HTTP/3 确实不基于TCP而是基于UDP了，但是加了一层QUIC来实现可靠性
...
epoll在内核维护一颗红黑树，一个就绪链表，一个等待队列

红黑树中存储所有被监控的fd  
就绪链表存储ready的fd

当把一个fd注册到epoll中时，会在这个fd对应的对象里注册一个回调函数

例如：  
一个socket的fd注册进epoll之后，内核会把epoll的回调函数加到socket的等待队列上  
当网卡接收到数据->TCP协议栈处理->放入socket buffer  
内核就会执行:
```cpp
sock_def_readable()
        ↓
...
RAII的核心思想是在构造函数中获取资源，在析构函数里释放资源  
RAII依赖的是C++的确定性析构，即对象离开作用域之前一定会执行析构函数  

可以用RAII的思想去包装一些需要管理的对象，比如锁，fd，文件，网络资源等
...
RVO（Return value optimization）是指返回值优化  
编译器在返回对象时，直接在目标位置构造，避免拷贝或移动
...
`unique_ptr`的独占所有权是通过删除拷贝构造和拷贝赋值实现的
...
**注**：一定要记住这里是有两个析构函数的，一个是类本身的析构函数，一个是shared_ptr的析构函数

在这个例子中，当main函数退出时，局部变量a销毁，此时会调用shared_ptr\<A\>的析构函数而不是类型A的析构函数，而shared_ptr的析构函数只是将引用计数减一而不会像A的析构函数一样去调用shared_ptr\<B\>的析构函数

## 怎么在main函数开始前打印一段日志

常见的做法是利用**全局对象先于main()构造**的这个规则  
创建一个类，在构造函数中打印日志，然后声明这个类的一个全局对象
...
public:
    Logger() {
        std::cout << "Program is starting before main()" << std::endl;
    }
};
...
虚表通常时编译器生成的全局静态数据，通常存放在程序的只读数据区
...
可以通过将operator new私有或者直接删除来使得对象只能在栈上分配： 
```cpp
class A {
public:
    A() = default;
...
当你实现了析构函数时，系统就不会自动生成移动构造函数和移动赋值运算符
...
保证一个类在程序中只有一个实例，并提供一个全局访问点

实现时核心特点：
- 私有化构造方法，防止外部通过new直接实例化对象
- 删除拷贝构造函数和拷贝赋值
- 类内部自行创建一个私有**静态**变量，保存该类的唯一实例
...
**注：** 访问单例对象的时候，要通过提供的静态函数来访问：
```cpp
single::get_instance().do_something();
```

创建一个静态实例是因为：
- 如果不是静态变量那就在函数中只是一个普通的局部变量，函数结束后就会销毁
- static局部变量保证只会创建一次，多次调用函数得到的实例都是同一个实例
